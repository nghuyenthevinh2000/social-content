...
Usage:
    uv run --project . .agents/hooks/file-selector/lint_readme_tree.py --changed-only   # Checks directories modified in git
    uv run --project . .agents/hooks/file-selector/lint_readme_tree.py --all            # Full tree validation
"""
...
def main():
    ...
    if all_violations:
        ...
        for v in all_violations:
            print(f"  • {v}")
        print("\nRun `uv run .agents/hooks/file-selector/sync_readme_tree.py <dir>` or manually add frontmatter to resolve.")
        sys.exit(1)
    else:
